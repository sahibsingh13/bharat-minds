...
export default function Settings() {
  ...
  return (
    <div>
      <button onClick={() => setOpen(true)} className="text-xs px-3 py-1.5 rounded-lg border border-gray-300 bg-gray-100 hover:bg-gray-200 hover:border-gray-400 transition-colors">⚙️ Preferences</button>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-2xl h-[90dvh] sm:h-[80dvh] rounded-lg border border-gray-200 bg-white text-black p-0 shadow-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
              <h2 className="text-sm font-semibold">Preferences</h2>
              <button onClick={() => setOpen(false)} className="text-xs opacity-75 hover:opacity-100">Close</button>
            </div>
            <div className="flex h-[calc(100%-44px)]">
              <div className="w-40 border-r border-gray-200 p-3 space-y-1 text-xs shrink-0">
                <button onClick={() => setTab('profile')} className={`w-full text-left px-3 py-2 rounded ${tab==='profile'?'bg-blue-100 text-blue-900':''}`}>Profile</button>
                <button onClick={() => setTab('models')} className={`w-full text-left px-3 py-2 rounded ${tab==='models'?'bg-blue-100 text-blue-900':''}`}>Models</button>
                <button onClick={() => setTab('keys')} className={`w-full text-left px-3 py-2 rounded ${tab==='keys'?'bg-blue-100 text-blue-900':''}`}>API Keys</button>
              </div>
              <div className="flex-1 p-4 overflow-y-auto">
                ...
                {tab==='keys' && (
                  <div>
                    <p className="text-[10px] text-gray-500 mb-4">Keys are stored locally in your browser via localStorage and sent only with your requests. Do not hardcode keys in code.</p>
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-xs font-medium">Gemini API Key</label>
                      <a href="https://aistudio.google.com/app/u/5/apikey?pli=1" target="_blank" rel="noopener noreferrer" className="text-[10px] px-2 py-1 rounded-md bg-blue-500 text-white border border-blue-500/20 hover:bg-blue-600 transition-colors">Get API key</a>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <a href="https://openrouter.ai/sign-in?redirect_url=https%3A%2F%2Fopenrouter.ai%2Fsettings%2Fkeys" target="_blank" rel="noopener noreferrer" className="text-[10px] px-2 py-1 rounded-md bg-blue-500 text-white border border-blue-500/20 hover:bg-blue-600 transition-colors">Get API key</a>
                    </div>
                    ...
                    <div className="flex gap-3 justify-end mt-6">
                      <button onClick={() => setOpen(false)} className="px-3 py-1.5 rounded-lg bg-gray-100 border border-gray-300 hover:bg-gray-200 transition-colors text-xs">Close</button>
                      <button onClick={save} className="px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white border border-blue-500/20 transition-colors text-xs">Save Keys</button>
                    </div>
                  </div>
                )}
              </div>
