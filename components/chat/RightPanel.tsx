...
export default function RightPanel({
  ...
  // Sync username when panel opens
  useEffect(() => {
    if (open) {
      try {
    ...
    <div className={`fixed right-0 top-0 h-full w-full sm:w-[380px] z-40 transition-transform ${open ? 'translate-x-0' : 'translate-x-full'}`}>
      ...
      <aside className="absolute right-0 top-0 h-full w-full sm:w-[380px] bg-white border-l border-gray-200 flex flex-col">
        <div className="p-3 border-b border-gray-200 flex items-center justify-between">
          <div className="text-xs font-semibold text-gray-800">Models & Settings</div>
          <button onClick={onClose} className="p-2 rounded bg-gray-100 hover:bg-gray-200 cursor-pointer"><X size={14} /></button>
        </div>
        <div className="p-3 space-y-3 overflow-y-auto">
          <div>
            <div className="text-[10px] uppercase opacity-60 mb-1 text-gray-600">Welcome</div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={saveName} className="text-[10px] px-2 py-1 rounded bg-blue-500 text-white border border-blue-500/20 hover:bg-blue-600 cursor-pointer">Save</button>
            </div>
          </div>
          <div>
            <div className="text-[10px] uppercase opacity-60 mb-1 text-gray-600">Chats</div>
            <div className="space-y-1">
              <button onClick={newChat} className="w-full text-left text-[10px] px-2 py-1 rounded bg-blue-500/15 border border-blue-400/20 hover:bg-blue-500/25 cursor-pointer inline-flex items-center gap-2">
                <Plus size={12} /> New Chat
              </button>
              <div className="max-h-[180px] overflow-y-auto pr-1 space-y-1">
                {threads.map((t) => (
                  <div key={t.id} className={`group flex items-center gap-1 rounded ${t.id===activeId ? 'bg-blue-100' : 'hover:bg-gray-100'}`}>
          ...
          <div>
            <div className="text-[10px] uppercase opacity-60 mb-1 text-gray-600">Models</div>
            <ModelSelector selectedIds={selectedIds} onToggle={onToggle} />
          </div>
          <div>
            <div className="text-[10px] uppercase opacity-60 mb-1 text-gray-600">Settings</div>
            <Settings />
          </div>
        </div>
      </aside>
