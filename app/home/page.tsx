      ...
      <aside className="hidden md:flex w-56 shrink-0 flex-col border-r border-gray-200 bg-white">
        <div className="p-2 border-b border-gray-200 flex items-center justify-between">
          ...
          <button
            ...
            className="relative h-6 w-6 rounded-full border border-blue-400/30 overflow-hidden cursor-pointer"
            title="Open settings"
          >
        ...
        <div className="flex-1 overflow-y-auto p-1 space-y-1">
          {threads.length === 0 && <div className="text-xs mobile-text-xs opacity-60 p-2 text-gray-600">No chats yet</div>}
          {threads.map(t => {
            const isActive = t.id === activeId;
            return (
              <div key={t.id} className={`group flex items-center gap-1 rounded-md ${isActive ? 'bg-blue-50' : 'bg-transparent hover:bg-gray-50'}`}>
                <button
                  onClick={() => setActiveId(t.id)}
                  className={`flex-1 text-left px-2 py-1.5 text-xs mobile-text-xs rounded-md cursor-pointer ${isActive ? 'text-blue-600' : 'text-gray-700'}`}
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-1 text-red-400 hover:text-red-300 cursor-pointer"
                  title="Delete chat"
                >
                  <Trash2 size={12} />
                </button>
        ...
        <div className="sticky top-0 z-10 bg-black/70 backdrop-blur border-b border-white/10 px-3 py-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setRightOpen(v => !v)} className="h-6 w-6 rounded-full border border-white/15 bg-white/5 hover:bg-white/10 cursor-pointer inline-flex items-center justify-center" title="Settings">
                <Cog size={12} />
              </button>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4" id="messages-container">
          {turns.length === 0 && selectedModels.length === 0 && (
            <div className="max-w-2xl mx-auto text-center text-zinc-400 mt-10">
              ...
              <div className="mt-3 text-xs mobile-text-xs">Start a conversation from the settings panel and compare answers.</div>
            </div>
          )}
          {turns.length === 0 && selectedModels.length > 0 && (
            <div className="max-w-2xl mx-auto text-center text-zinc-400 mt-10">
              ...
              <div className="mt-3 text-xs mobile-text-xs">Models selected. Ask your first question to get started.</div>
            </div>
          )}
            ...
            {turns.map((row, idx) => (
              <div key={idx} className="space-y-2">
                <div className="rounded-lg bg-white/5 border border-white/10 p-3">
                  <div className="text-xs mobile-text-xs text-zinc-300"><span className="opacity-60">You:</span> {row.user.content}</div>
                </div>
                {/* Answers inline tabs when 2+ models selected */}
                {row.answers.length === 0 ? (
                  <div className="rounded-lg border border-white/10 p-3 text-xs mobile-text-xs">
                    <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="flex gap-1 mb-2">
                      {selectedModels.map((m, i) => (
                        <a key={m.id} href={`#turn-${idx}-${m.id}`} className="text-[10px] mobile-text-xs px-1.5 py-0.5 rounded bg-white/10 border border-white/15 hover:bg-white/15">
                          {m.label}
                        </a>
                      ))}
                    </div>
                      ...
                      {selectedModels.map((m) => {
                        ...
                        return (
                          <div id={`turn-${idx}-${m.id}`} key={m.id} className="rounded-lg p-3 ring-1 bg-white/5 ring-white/10">
                            <div className="mb-1 text-[10px] mobile-text-xs inline-flex items-center gap-2">
                              <span className="px-1.5 py-0.5 rounded-full border border-white/20 text-zinc-200">{m.label}</span>
                            </div>
                            <div className="prose prose-invert max-w-none text-xs mobile-text-xs">
                  ...
                  row.answers.map((ans) => {
                    ...
                    return (
                      <div key={`${idx}-${ans.modelId}-${ans.ts}`} className={`rounded-lg p-3 ring-1 ${m?.good ? 'bg-amber-400/10 ring-amber-300/30' : 'bg-white/5 ring-white/10'}`}>
                        <div className="mb-1 text-[10px] mobile-text-xs inline-flex items-center gap-2">
                          <span className={`px-1.5 py-0.5 rounded-full border ${m?.good ? 'border-amber-300/40 text-amber-200' : 'border-white/20 text-zinc-200'}`}>{m?.label || ans.modelId}</span>
